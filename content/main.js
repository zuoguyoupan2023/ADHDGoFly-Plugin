...
class ADHDHighlighter {
  ...
  ensureAiSettingPanel() {
    ...
    const renderVocabSummary = () => {
      ...
      vocabResult.innerHTML = `<p>本轮复习完成，下面是总览。</p><div class="agf-vocab-list"><table><thead><tr><th>词汇</th><th>词性</th><th>对应语言含义</th><th>对应语言例句</th></tr></thead><tbody>${vocabCards.map(item => { const latin = /[A-Za-z]/.test(String(item.word || '')); const meaning = latin ? (item.definition || item.englishMeaning || item.meaning || '') : (item.meaning || item.definition || item.englishMeaning || ''); return `<tr><td>${p1Esc(item.word)}</td><td>${p1Esc(item.pos || item.partOfSpeech || '待补充')}</td><td>${p1Esc(meaning)}</td><td>${p1Esc(item.example || '')}</td></tr>`; }).join('')}</tbody></table></div>`;
    };
    const renderVocabCard = () => {
      ...
      vocabResult.innerHTML = `<article class="agf-vocab-card"><h3>${p1Esc(card.word)}</h3><p><strong>词性：</strong>${p1Esc(card.pos || card.partOfSpeech || '待补充')}</p><p><strong>对应语言含义：</strong>${p1Esc(meaning)}</p><p><strong>对应语言例句：</strong>${p1Esc(card.example || '')}</p><button class="agf-task-btn" data-vocab-sentence="${vocabIndex}">造句</button></article><div class="agf-vocab-review-current"><span>第 ${vocabIndex + 1} / ${vocabCards.length} 词</span><button id="agfVocabRemember">我记住了</button><button id="agfVocabForget">还不熟</button></div>`;
