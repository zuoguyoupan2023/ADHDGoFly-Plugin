  ...
  async handleMessage(message, sender, sendResponse) {
    try {
      switch (message.action) {
        ...
        case 'getUserDictionaryManager':
          sendResponse({ success: true, available: this.dictionaryManager && this.dictionaryManager.getUserDictionaryManager() });
          break;

        case 'userDict_getAllWords':
          const userDictManager = this.dictionaryManager?.getUserDictionaryManager();
          if (userDictManager) {
            const words = await userDictManager.getAllWords();
            sendResponse({ success: true, words });
          } else {
            sendResponse({ success: false, error: 'User dictionary manager not available' });
          }
          break;

        case 'userDict_addWord':
          const addManager = this.dictionaryManager?.getUserDictionaryManager();
          if (addManager) {
            const result = await addManager.addWord(message.word, message.language, message.pos, message.notes);
        ...
        case 'userDict_editWord':
          const editManager = this.dictionaryManager?.getUserDictionaryManager();
          if (editManager) {
            const result = await editManager.editWord(message.wordId, message.updates);
            sendResponse({ success: true, result });
          } else {
            sendResponse({ success: false, error: 'User dictionary manager not available' });
          }
        ...
        case 'userDict_deleteWord':
          const deleteManager = this.dictionaryManager?.getUserDictionaryManager();
          if (deleteManager) {
            const result = await deleteManager.deleteWord(message.wordId);
            sendResponse({ success: true, result });
          } else {
            sendResponse({ success: false, error: 'User dictionary manager not available' });
          }
        ...
        case 'userDict_searchWords':
          const searchManager = this.dictionaryManager?.getUserDictionaryManager();
          if (searchManager) {
            const results = await searchManager.searchWords(message.query, message.language);
        ...
        case 'userDict_importDictionary':
          const importManager = this.dictionaryManager?.getUserDictionaryManager();
          if (importManager) {
            const result = await importManager.importDictionary(message.data);
            sendResponse({ success: true, result });
          } else {
            sendResponse({ success: false, error: 'User dictionary manager not available' });
          }
        ...
        case 'userDict_exportDictionary':
          const exportManager = this.dictionaryManager?.getUserDictionaryManager();
          if (exportManager) {
            const data = await exportManager.exportDictionary();
        ...
        case 'userDict_getStats':
          const statsManager = this.dictionaryManager?.getUserDictionaryManager();
          if (statsManager) {
        ...
        case 'userDict_hasWord':
          const hasWordManager = this.dictionaryManager?.getUserDictionaryManager();
          if (hasWordManager) {
            const exists = await hasWordManager.hasWord(message.language, message.word);
        ...
        case 'userDict_getWord':
          const getWordManager = this.dictionaryManager?.getUserDictionaryManager();
          if (getWordManager) {
            const wordData = await getWordManager.getWord(message.language, message.word);
            sendResponse({ success: true, wordData });
          } else {
            sendResponse({ success: false, error: 'User dictionary manager not available' });
          }
          break;
