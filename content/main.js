...
class ADHDHighlighter {
  ...
  ensureAiSettingPanel() {
    ...
    style.textContent = `
      ...
      #agfAiSettingOverlay #agfCtxParagraph{display:none!important}
        ...
        <div class="agf-ai-content">
          ...
          <div class="agf-ai-view-module" id="agfAiViewVocab" style="display:none">
            <div class="agf-module-card">
              ...
              <div class="agf-module-actions"><button id="agfVocabSave" class="agf-task-btn">保存</button><button id="agfVocabSaveDictionary" class="agf-task-btn">保存为词典</button><button id="agfVocabReset">重置本轮</button></div><div id="agfVocabHistory" class="agf-module-history" style="display:none"></div>
            </div>
          </div>
    ...
    const vocabSave = document.getElementById('agfVocabSave');
    ...
    chatNewBtn.type = 'button'; chatNewBtn.className = 'agf-send agf-workspace-clear'; chatNewBtn.dataset.i18n = 'jixia.ui.newConversation'; chatNewBtn.dataset.i18nTitle = 'jixia.ui.newConversationTitle';
    const updateChatNewLabel = () => { chatNewBtn.textContent = window.i18n?.t?.('jixia.ui.newConversation') || 'New chat'; chatNewBtn.title = window.i18n?.t?.('jixia.ui.newConversationTitle') || 'Start a new chat and keep history'; };
    updateChatNewLabel();
    document.addEventListener('languageChanged', updateChatNewLabel, { passive: true });
    ...
    const workspaceSaveRegistry = new Map();
    ...
    const saveWorkspace = async (id, manual = true) => { const entry = workspaceSaveRegistry.get(id); if (!entry || entry.saving) return false; const snapshot = JSON.stringify(entry.getSnapshot()); if (!snapshot || snapshot === 'null') return false; entry.saving = true; try { await entry.save(JSON.parse(snapshot)); entry.lastSaved = snapshot; if (manual) showSaveToast(tr('jixia.ui.saveSuccess', 'Saved')); return true; } finally { entry.saving = false; } };
    ...
    const createWorkspaceClearButton = (label = tr('jixia.ui.clearWorkspace', 'Clear workspace')) => {
    ...
    const setView = (which) => {
      ...
      const moduleToStore = which === 'p1' ? 'reading' : which;
      if (['chat','reading','writing','quiz','explain','vocab','image','chart'].includes(moduleToStore)) {
        try { chrome.storage.local.set({ agfJixiaLastModule: moduleToStore }); } catch (_) {}
      }
    ...
    const chartDataControls = chartView.querySelector('#agfChartDataControls');
    const chartUnits = chartView.querySelector('#agfChartUnits');
    const chartSeriesType = chartView.querySelector('#agfChartSeriesType');
    ...
    const renderEchartsCanvas = context => {
      ...
      const switchType = type => { const option = instance.getOption(); option.series = (model.series || []).map(item => ({ name: item.name, type, data: type === 'pie' ? (item.data || []).map(point => ({ name: point.label, value: Number(point.value) })) : (item.data || []).map(point => Number(point.value)), smooth: type === 'line', label: type === 'pie' ? { show: true } : undefined, ...(type === 'pie' ? { center: ['32%', '56%'], radius: ['0%', '30%'] } : {}) })); option.xAxis = type === 'pie' ? [] : [{ type: 'category', data: labels }]; option.yAxis = type === 'pie' ? [] : [{ type: 'value', name: model.units || '' }]; option.dataZoom = type === 'pie' ? [] : (labels.length > 8 ? [{ type: 'inside' }, { type: 'slider', bottom: 18 }] : []); option.legend = type === 'pie' ? { type: 'scroll', orient: 'vertical', right: 18, top: 96, bottom: 24, width: 190 } : { type: 'scroll', top: 58 }; option.tooltip = { ...tooltip, trigger: type === 'pie' ? 'item' : 'axis' }; instance.setOption(option, true); };
      const barIcon = 'path://M3,18 L3,10 L6,10 L6,18 Z M8,18 L8,5 L11,5 L11,18 Z M13,18 L13,8 L16,8 L16,18 Z';
      const lineIcon = 'path://M2,15 L7,10 L11,13 L17,5 M15,5 L17,5 L17,7';
      const pieIcon = 'path://M10,2 A8,8 0 1,1 2,10 L10,10 Z M10,2 L10,10 L18,10 A8,8 0 0,0 10,2 Z';
      instance.setOption({ animation: false, title: { text: model.title || '数据图表', subtext: [model.units, model.timeRange?.start && model.timeRange?.end ? `${model.timeRange.start}–${model.timeRange.end}` : ''].filter(Boolean).join(' · '), top: 10, left: 18 }, tooltip, legend: hasPie ? { type: 'scroll', orient: 'vertical', right: 18, top: 96, bottom: 24, width: 190 } : { type: 'scroll', top: 58 }, toolbox: { top: 10, right: 18, feature: { myPie: { show: true, title: '切换饼图', icon: pieIcon, onclick: () => switchType('pie') }, myBar: { show: true, title: '切换柱状图', icon: barIcon, onclick: () => switchType('bar') }, myLine: { show: true, title: '切换折线图', icon: lineIcon, onclick: () => switchType('line') }, dataView: { readOnly: true }, restore: {}, saveAsImage: {} } }, grid: { left: 56, right: 24, bottom: 72, top: 92, containLabel: true }, xAxis: hasPie ? undefined : { type: 'category', data: labels }, yAxis: hasPie ? undefined : { type: 'value', name: model.units || '' }, dataZoom: !hasPie && labels.length > 8 ? [{ type: 'inside' }, { type: 'slider', bottom: 18 }] : [], series: hasPie ? series.map(item => ({ ...item, center: ['32%', '56%'], radius: ['0%', '30%'] })) : series });
    ...
    const renderChartPreview = async () => {
      ...
      if (chartRenderer) chartRenderer.value = currentChartContext.renderer === 'mermaid' ? 'mermaid' : (currentChartContext.renderer === 'rough' ? 'rough' : (currentChartContext.renderer === 'echarts' ? 'echarts' : 'svg'));
      if (chartTheme) chartTheme.value = currentChartContext.theme || 'system';
      if (chartIntent) chartIntent.value = currentChartContext.intent === 'data' ? 'data' : (currentChartContext.intent || 'relationship');
      chartMeta.textContent = `${currentChartContext.source} · ${new Date(currentChartContext.updatedAt).toLocaleString()}`;
      const isDataChart = currentChartContext.intent === 'data' || currentChartContext.viewType === 'data_flow';
      if (chartDataControls) chartDataControls.style.display = isDataChart ? 'flex' : 'none';
      if (isDataChart) {
        if (chartUnits) chartUnits.value = currentChartContext.chartModel.units || '';
        if (chartSeriesType) chartSeriesType.value = currentChartContext.chartModel.series?.[0]?.type || 'bar';
      }
    ...
    if (chartIntent) chartIntent.onchange = () => { const isData = chartIntent.value === 'data'; if (chartDataControls) chartDataControls.style.display = isData ? 'flex' : 'none'; if (isData && chartRenderer) chartRenderer.value = 'echarts'; };
    if (chartUnits) chartUnits.oninput = () => { if (!currentChartContext?.chartModel) return; currentChartContext.chartModel.units = chartUnits.value.trim(); currentChartContext.updatedAt = Date.now(); renderChartPreview(); };
    if (chartSeriesType) chartSeriesType.onchange = () => { if (!currentChartContext?.chartModel?.series?.length) return; currentChartContext.chartModel.series.forEach(series => { series.type = chartSeriesType.value; }); currentChartContext.updatedAt = Date.now(); renderChartPreview(); };
    ...
    const chartSourceLinks = chartView.querySelector('#agfChartSourceLinks') || (() => { const element = document.createElement('div'); element.id = 'agfChartSourceLinks'; element.style.cssText = 'margin:8px 0;font-size:12px;color:#687386'; chartHistory?.before(element); return element; })();
    const openChartSource = source => {
      const locator = String(source.locator || '').trim();
      const target = locator ? document.querySelector(`#${CSS.escape(locator)}`) || document.querySelector(`[data-source-locator="${CSS.escape(locator)}"]`) : null;
      if (target) { target.scrollIntoView({ behavior: 'smooth', block: 'center' }); target.style.outline = '3px solid #f59e0b'; setTimeout(() => { target.style.outline = ''; }, 1800); return; }
      if (source.url && /^https?:\/\//i.test(source.url)) window.open(`${source.url}${locator ? `#${encodeURIComponent(locator)}` : ''}`, '_blank', 'noopener');
      else if (source.text) {
        const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT); let node; while ((node = walker.nextNode())) if (node.nodeValue.includes(source.text)) { node.parentElement?.scrollIntoView({ behavior: 'smooth', block: 'center' }); node.parentElement?.style.setProperty('outline', '3px solid #f59e0b'); break; }
      }
    };
    const renderChartSourceLinks = () => {
      if (!chartSourceLinks || !currentChartContext) return;
      const links = window.AgfChartModel?.buildSourceLinks?.(currentChartContext) || [];
      chartSourceLinks.innerHTML = '<strong>来源定位</strong> ';
      if (!links.length) { chartSourceLinks.append('暂无来源定位'); return; }
      links.slice(0, 20).forEach((source, index) => { const button = document.createElement('button'); button.type = 'button'; button.className = 'agf-task-btn'; button.style.cssText = 'margin:3px;padding:3px 7px;font-size:11px'; button.textContent = `${source.scope} · ${source.label || '来源'}`; button.title = source.locator || source.url || source.text || ''; button.onclick = () => { const id = source.scope === 'node' ? source.id : ''; const edgeIndex = source.scope === 'edge' ? Number(String(source.id).split('#')[1]) : -1; if (id) { chartHistoryState.selected = new Set([id]); chartHistoryState.selectedEdges.clear(); renderChartPreview(); } else if (edgeIndex >= 0) { chartHistoryState.selectedEdges = new Set([edgeIndex]); chartHistoryState.selected.clear(); renderChartPreview(); } openChartSource(source); }; chartSourceLinks.append(button); if (index === 19 && links.length > 20) chartSourceLinks.append(' …'); });
    };
    const renderChartHistoryVersions = row => {
      const versions = Array.isArray(row.versionHistory) ? row.versionHistory : [];
      if (!versions.length) return '<span style="font-size:11px;color:#687386">暂无版本快照</span>';
      return `<select data-version-select="${row.id}" class="agf-select" style="max-width:130px"><option value="">选择版本</option>${versions.slice().reverse().map(version => `<option value="${version.version}">v${version.version} · ${String(version.reason || '保存')}</option>`).join('')}</select><button class="agf-task-btn" data-version-restore="${row.id}">恢复</button><button class="agf-task-btn" data-version-compare="${row.id}">比较</button>`;
    };
    const loadChartHistory = async () => { try { const rows = await AgfChartWorkspace.list(); chartHistory.innerHTML = rows.length ? rows.slice(0, 12).map(row => `<div class="agf-chart-history-row" data-history-row="${row.id}" style="flex-wrap:wrap"><span>${String(row.chartModel?.title || '未命名')} · ${row.intent === 'timeline' ? '时间线' : '关系图'} · v${row.version || 1}</span><button class="agf-task-btn" data-chart-load="${row.id}">打开</button><div style="width:100%">${renderChartHistoryVersions(row)}</div><pre data-version-diff="${row.id}" style="display:none;white-space:pre-wrap;margin:4px 0;width:100%;font-size:11px"></pre></div>`).join('') : '<span style="font-size:12px;color:#687386">暂无保存记录</span>'; chartHistory.querySelectorAll('[data-chart-load]').forEach(button => button.onclick = async () => { currentChartContext = await AgfChartWorkspace.get(button.dataset.chartLoad); renderChartPreview(); }); chartHistory.querySelectorAll('[data-version-restore]').forEach(button => button.onclick = async () => { const row = await AgfChartWorkspace.get(button.dataset.versionRestore); const select = chartHistory.querySelector(`[data-version-select="${CSS.escape(button.dataset.versionRestore)}"]`); if (!select?.value) { chartNotice.textContent = '请先选择要恢复的版本'; return; } currentChartContext = AgfChartWorkspace.restoreVersion(row, select.value); currentChartContext.changeReason = `恢复版本 ${select.value}`; renderChartPreview(); chartNotice.textContent = `已恢复版本 v${select.value}，保存后生效`; }); chartHistory.querySelectorAll('[data-version-compare]').forEach(button => button.onclick = async () => { const row = await AgfChartWorkspace.get(button.dataset.versionCompare); const select = chartHistory.querySelector(`[data-version-select="${CSS.escape(button.dataset.versionCompare)}"]`); const diff = chartHistory.querySelector(`[data-version-diff="${CSS.escape(button.dataset.versionCompare)}"]`); if (!select?.value || !diff) { chartNotice.textContent = '请先选择一个版本'; return; } const selected = (row.versionHistory || []).find(version => String(version.version) === select.value); const current = row.versionHistory?.[row.versionHistory.length - 1] || row; const result = AgfChartWorkspace.compareVersions(selected, current); diff.style.display = 'block'; diff.textContent = result.changed ? result.changes.map(change => `${change.field}\n旧：${JSON.stringify(change.before)}\n新：${JSON.stringify(change.after)}`).join('\n\n') : '与当前版本没有差异'; }); } catch (error) { chartNotice.textContent = error.message || '无法读取图表历史'; } };
    ...
    const buildChartPrompt = (intent, material, options = {}) => {
      ...
      const config = {
        ...
        data: { name: '数据图表', schema: 'series:[{name,type:"bar|line|pie|table",data:[{label,value}]}],units,timeRange', rule: '只使用输入中明确出现的数值；缺少可靠数值时返回 type:"table" 或在 warnings 说明无法绘制趋势，绝不编造数据。' },
    ...
    chartButtons.aiEdit.onclick = async () => {
      if (!currentChartContext) { chartNotice.textContent = '请先生成或打开一个图表。'; return; }
      const request = String(chartAiEditInput?.value || '').trim();
      if (!request) { chartNotice.textContent = '请输入要修改的内容。'; chartAiEditInput?.focus(); return; }
      const original = chartSnapshot();
      try {
        chartButtons.aiEdit.disabled = true;
        chartNotice.textContent = '正在让 AI 修改结构化图表…';
        const payload = {
          chartType: currentChartContext.viewType || currentChartContext.intent,
          intent: currentChartContext.intent,
          renderer: currentChartContext.renderer,
          theme: currentChartContext.theme,
          metadata: { id: currentChartContext.id, source: currentChartContext.source, sourceRefs: currentChartContext.sourceRefs || [], summary: currentChartContext.summary || '', warnings: currentChartContext.chartModel?.warnings || [] },
          chartModel: currentChartContext.chartModel,
          userRequest: request
        };
        ...
        if (!checked.valid) throw new Error(`AI 返回的图表模型无效：${checked.errors.join('；')}`);
        rememberChart();
        currentChartContext = checked.value;
        ...
        chartNotice.textContent = error.message || 'AI 修改图表失败';
      } finally {
        chartButtons.aiEdit.disabled = false;
      }
    };
    ...
    chartButtons.addLane.onclick = () => { if (!currentChartContext?.chartModel?.nodes) return; rememberChart(); const nodes = currentChartContext.chartModel.nodes; const existing = nodes.filter(node => node.isLane && !node.hidden && node.laneKey === '主流程').length; const id = `lane-${Date.now().toString(36)}`; nodes.push({ id, label: `主流程 ${existing + 1}`, laneKey: '主流程', rowIndex: existing, isLane: true, description: '泳道', sourceRefs: [] }); currentChartContext._laneNodesInitialized = true; currentChartContext.updatedAt = Date.now(); chartHistoryState.selected = new Set([id]); renderChartPreview(); };
    ...
    this.__openJixiaModule = (request = {}) => {
      ...
      else if (request.module === 'reading') setView('reading');
      else if (request.module === 'writing') setView('writing');
      else if (request.module === 'explain') setView('explain');
      else if (request.module === 'vocab') setView('vocab');
      else if (request.module === 'image') setView('image');
      else if (request.module === 'chart') setView('chart');
      else showChat();
    };
    ...
    if (pendingOpen) {
      ...
      try {
        chrome.storage.local.get(['agfJixiaLastModule'], res => {
          ...
          else if (['reading','writing','explain','vocab','image','chart'].includes(last)) setView(last);
          else showChat();
        });
      } catch (_) {
        showChat();
    ...
    const renderP1Result = (kind, result, ctx) => {
      const labels = { structured: tr('jixia.ui.structuredReading', 'Structured reading'), writing: tr('jixia.ui.writingResult', 'Writing result'), fact: tr('jixia.ui.factCheck', 'Fact check') }; p1Title.textContent = labels[kind]; p1Meta.textContent = `${String(ctx?.pageTitle || tr('jixia.ui.currentArticle', 'Current article')).slice(0, 40)} · ${new Date().toLocaleTimeString()}`;
      if (kind === 'structured') { p1Result.innerHTML = `<h3>${p1Esc(tr('jixia.ui.thesis', 'Thesis'))}</h3><p>${p1Esc(result.thesis)}</p><h3>${p1Esc(tr('jixia.ui.argumentsEvidence', 'Arguments and evidence'))}</h3>${(result.arguments || []).map(item => `<article><strong>${p1Esc(item.claim)}</strong><p>${p1Esc(tr('jixia.ui.evidence', 'Evidence'))}: ${p1Esc(p1Text(item.evidence))}</p><small>${p1Esc(tr('jixia.ui.confidence', 'Confidence'))}: ${p1Esc(item.confidence ?? tr('jixia.ui.notProvided', 'Not provided'))}</small></article>`).join('') || `<p>${p1Esc(tr('jixia.ui.noArguments', 'No arguments found.'))}</p>`}<h3>${p1Esc(tr('jixia.ui.causalRelations', 'Causal relations'))}</h3>${(result.causalRelations || []).map(item => `<p>${p1Esc(item.cause)} -> ${p1Esc(item.effect)}<br>${p1Esc(tr('jixia.ui.evidence', 'Evidence'))}: ${p1Esc(item.evidence)}</p>`).join('') || `<p>${p1Esc(tr('jixia.ui.noCausalRelations', 'No clear causal relations found.'))}</p>`}<h3>${p1Esc(tr('jixia.ui.controversies', 'Controversies'))}</h3>${(result.controversies || []).map(item => `<p>${p1Esc(item.claim)}<br>${p1Esc(tr('jixia.ui.position', 'Position'))}: ${p1Esc(p1Text(item.positions))}<br>${p1Esc(tr('jixia.ui.uncertainty', 'Uncertainty'))}: ${p1Esc(item.uncertainty)}</p>`).join('') || `<p>${p1Esc(tr('jixia.ui.noControversies', 'No controversies found.'))}</p>`}`; }
      else if (kind === 'writing') { p1Result.innerHTML = `<h3>${p1Esc(result.title || tr('jixia.ui.writingResult', 'Writing result'))}</h3>${(result.sections || []).map(section => `<article><h4>${p1Esc(section.heading)}</h4><p>${p1Esc(section.content)}</p><ul>${(section.bullets || []).map(bullet => `<li>${p1Esc(bullet)}</li>`).join('')}</ul></article>`).join('')}<h3>${p1Esc(tr('jixia.ui.writingDraft', 'Draft'))}</h3><pre style="white-space:pre-wrap">${p1Esc(result.draft)}</pre>${(result.citations || []).length ? `<h3>${p1Esc(tr('jixia.ui.citations', 'Citation cards'))}</h3>${result.citations.map(citation => `<blockquote>${p1Esc(citation.quote || citation.text)}<br><small>${p1Esc(citation.use || citation.locator)}</small></blockquote>`).join('')}` : ''}`; }
      else { const names = { fact: tr('jixia.ui.claimFact', 'Fact'), inference: tr('jixia.ui.claimInference', 'Inference'), opinion: tr('jixia.ui.claimOpinion', 'Opinion'), unverified: tr('jixia.ui.claimUnverified', 'Unverified') }; p1Result.innerHTML = (result.claims || []).map(item => `<article><strong>${p1Esc(names[item.classification] || item.classification)}</strong>: ${p1Esc(item.text)}<p>${p1Esc(tr('jixia.ui.evidence', 'Evidence'))}: ${p1Esc(item.evidence)}</p><small>${p1Esc(item.reason)} · ${p1Esc(tr('jixia.ui.confidence', 'Confidence'))} ${Math.round(Number(item.confidence || 0) * 100)}%${item.locator ? ` · ${p1Esc(item.locator)}` : ''}</small></article>`).join('') || `<p>${p1Esc(tr('jixia.ui.noClaims', 'No classifiable claims found.'))}</p>`; }
    ...
    const factCheckModule = JixiaModules.createFactCheckModule({ context: jixiaContext, task: jixiaTask, languageHint: getJixiaLangHint, onResult: (result, ctx) => renderP1Result('fact', result, ctx) });
    const prepareP1 = kind => { p1ActionsEl.innerHTML = ''; p1Title.textContent = ({ structured: tr('jixia.ui.structuredReading', 'Structured reading'), writing: tr('jixia.ui.writingResult', 'Writing result'), fact: tr('jixia.ui.factCheck', 'Fact check') })[kind] || tr('jixia.ui.p1Title', 'Reading tool'); };
    const runP1 = async (kind, action) => { prepareP1(kind); setView('p1'); p1Result.innerHTML = `<p>${p1Esc(tr('jixia.ui.processingAnalysis', 'Analyzing, please wait...'))}</p>`; try { await action(); } catch (error) { p1Result.innerHTML = `<p>${p1Esc(error.message || error)}</p>`; } };
    ...
    if (readingSave) readingSave.onclick = () => saveReadingResult().catch(error => showToast(error.message || tr('jixia.ui.readingSaveFailed', 'Failed to save reading result')));
    ...
    readingClear.onclick = async () => {
      ...
      readingSceneResult = '';
      p1Title.textContent = tr('jixia.ui.readingTitle', 'Reading');
      p1Meta.textContent = '';
      p1Result.innerHTML = `<p>${p1Esc(tr('jixia.ui.selectScene', 'Choose a reading scene to begin.'))}</p>`;
      ...
      showToast(tr('jixia.ui.readingCleared', 'Reading workspace cleared. History is kept.'));
    };
    ...
    if (writingSave) writingSave.onclick = () => saveWritingResult().catch(error => showToast(error.message || tr('jixia.ui.writingSaveFailed', 'Failed to save writing result')));
    ...
    writingClear.onclick = async () => {
      ...
      if (writingResult) writingResult.innerHTML = `<p>${p1Esc(tr('jixia.ui.selectWritingScene', 'Choose a writing scene to begin.'))}</p>`;
      ...
      showToast(tr('jixia.ui.writingCleared', 'Writing workspace cleared. History is kept.'));
    };
    ...
    JixiaUiModules.bindChatEvents({ elements: { quickSummary: quickSummaryBtn, beginnerExplain: beginnerExplainBtn, translate: btnTranslate, structured: btnStructured, explain: btnExplain, outline: btnOutline, keywords: btnKeywords, tab: tabChat }, actions: {
      ...
      labels: { summary: translate('aiPanel.summary') || '总结', beginner: translate('aiPanel.beginnerExplain') || '保姆级解读', translate: translate('jixia.tasks.translate') || '翻译', structured: translate('aiPanel.structured') || '结构化摘要', explain: translate('aiPanel.explain') || '简明解释', outline: translate('aiPanel.outline') || '提取大纲', keywords: translate('aiPanel.keywords') || '提取关键词' },
      chatTasks: {
        summary: { title: translate('aiPanel.prompts.summaryTitle') || '帮我总结这篇文章: ' },
        translate: { title: 'Translate the following content while preserving terminology and paragraph structure.', prefix: translate('jixia.tasks.translate') || '翻译', extra: 'Output requirement: if the original text is Chinese, translate it into English; if the original text is not Chinese, translate it into Chinese.' },
