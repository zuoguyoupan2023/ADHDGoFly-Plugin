...
class ADHDHighlighter {
  ...
  ensureAiSettingPanel() {
    ...
    const chartModifyBox = chartView.querySelector('#agfChartModifyBox') || (() => { const box = document.createElement('div'); box.id = 'agfChartModifyBox'; box.style.cssText = 'display:flex;gap:8px;align-items:flex-start;margin-top:10px'; box.innerHTML = '<textarea id="agfChartModifyInput" class="agf-field" style="flex:1;min-height:54px;resize:vertical" placeholder="例如：增加一个 Redis，放在数据库前面，并把缓存未命中路径标红"></textarea><button id="agfChartModify" class="agf-task-btn" disabled>AI 修改图表</button>'; chartCanvas?.insertAdjacentElement('afterend', box); return box; })();
    const chartModifyInput = chartModifyBox?.querySelector('#agfChartModifyInput'); const chartModifyButton = chartModifyBox?.querySelector('#agfChartModify');
    const applyWorkspaceI18n = () => {
      const set = (element, key, fallback) => { if (element) element.textContent = t(key) || fallback; };
      set(chartView.querySelector('.agf-module-heading span'), 'chartWorkspace.title', '图表工作区');
      if (chartModifyButton) set(chartModifyButton, 'chartWorkspace.modify', 'AI 修改图表');
      if (chartModifyInput) chartModifyInput.placeholder = t('chartWorkspace.modifyPlaceholder') || '修改当前图表…';
      if (chartButtons.addLane) set(chartButtons.addLane, 'chartWorkspace.addLane', '添加泳道');
      set(imageView?.querySelector('.agf-module-heading span'), 'imageWorkspace.title', '图像工作区');
      set(imageChooseBtn, 'imageWorkspace.choose', '选择图片'); set(imageProcessSelected, 'imageWorkspace.process', '发送勾选图片识别'); set(imageAddToChat, 'imageWorkspace.addToChat', '添加到对话框'); set(imageWorkspaceRetry, 'imageWorkspace.retry', '重新识别'); set(imageWorkspaceClearBtn, 'imageWorkspace.clear', '清除工作区'); set(imageWorkspaceDeleteBtn, 'imageWorkspace.delete', '删除并清理历史'); set(imageWorkspaceExportBtn, 'imageWorkspace.export', '导出'); set(imageWorkspaceHistoryBtn, 'imageWorkspace.history', '识别历史');
      set(taskActions?.querySelector('#agfBtnTranslate'), 'taskBar.translate', '翻译'); set(taskActions?.querySelector('#agfBtnSelectionExplain'), 'taskBar.selectionExplain', '选区解释'); set(taskActions?.querySelector('#agfBtnVisionOcr'), 'taskBar.visionOcr', '图片识别/OCR'); set(taskActions?.querySelector('#agfBtnSpeak'), 'taskBar.speak', '朗读'); set(taskActions?.querySelector('#agfModuleHistoryBtn'), 'taskBar.history', '历史记录'); set(taskActions?.querySelector('.agf-task-label'), 'taskBar.label', '任务');
    };
    applyWorkspaceI18n();
    ...
    const renderChartPreview = async () => {
      ...
      if (chartModifyButton) chartModifyButton.disabled = false;
    ...
    chartModifyButton.onclick = async () => {
      if (!currentChartContext || !String(chartModifyInput?.value || '').trim()) return;
      const request = String(chartModifyInput.value).trim(); const base = { intent: currentChartContext.intent, viewType: currentChartContext.viewType, renderer: currentChartContext.renderer, theme: currentChartContext.theme, chartModel: currentChartContext.chartModel };
      try {
        chartNotice.textContent = '正在根据修改要求更新图表…'; chartModifyButton.disabled = true;
        const prompt = `你是图表修改器。不要输出Markdown或解释，只返回严格JSON。请根据当前图表和用户修改要求，返回修改后的完整 chartModel；保留未修改的节点、边、sourceRefs、warnings，不要直接输出 SVG。当前图表类型：${base.viewType || base.intent}。当前图表 JSON：${JSON.stringify(base.chartModel)}。用户修改要求：${request}。必须保证所有边的 source/target 都引用存在节点，节点 id 不重复；如果用户要求的事实无法从当前图表确定，可以使用通用知识，但不要编造外部来源。`;
        const output = await taixueTask.requestJsonText({ prompt, timeout: 90000, maxTokens: 8000, temperature: .2 }); const model = typeof output === 'string' ? AgfChartModel.parseJsonObject(output) : output;
        const checked = AgfChartModel.validateChartContext({ ...currentChartContext, chartModel: model }); if (!checked.valid) throw new Error(checked.errors.join('；'));
        rememberChart(); currentChartContext = { ...currentChartContext, chartModel: checked.value.chartModel, updatedAt: Date.now() }; chartModifyInput.value = ''; renderChartPreview(); chartNotice.textContent = 'AI 修改已应用，可继续编辑或撤销。';
      } catch (error) { chartNotice.textContent = error.message || 'AI 修改图表失败'; } finally { chartModifyButton.disabled = !currentChartContext; }
    };
