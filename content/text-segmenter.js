...
class TextSegmenter {
  ...
  /**
   * 法语文本分词处理
   * @param {string} text 待分词的法语文本
   * @param {Object} dictionary 法语词典
   * @param {Object} dictionaryManager 词典管理器（可选）
   * @returns {string} 分词后的HTML字符串
   */
  segmentFrLangText(text, dictionary, dictionaryManager = null) {
    if (!text || !dictionary) {
      return text || '';
    }
    ...
    tokens.forEach(token => {
      ...
      // 处理法语缩写形式（l', d', s', c', j', m', t', n'等）
      let processedToken = token;
      let prefix = '';
      
      // 检查是否有法语缩写前缀
      const frenchContractions = /^([ldjcmtn]')(.+)$/i;
      const contractionMatch = token.match(frenchContractions);
      
      if (contractionMatch) {
        prefix = contractionMatch[1]; // l', d', s'等
        processedToken = contractionMatch[2]; // 剩余部分
      }
      
      // 清理词汇（移除标点，转为小写）
      const cleanWord = this.cleanWord(processedToken);
      
      // 首先尝试精确匹配
      if (cleanWord && dictionary.words && dictionary.words[cleanWord]) {
        const entry = dictionary.words[cleanWord];
        const pos = entry.pos ? entry.pos[0] : null; // 取第一个词性
        const normalizedPos = this.normalizePartOfSpeech(pos);
        
        // 根据词性和高亮开关决定是否高亮
        const shouldHighlight = (
          (normalizedPos === 'noun' && this.highlightingToggles.noun) ||
          (normalizedPos === 'verb' && this.highlightingToggles.verb) ||
          (normalizedPos === 'adj' && this.highlightingToggles.adj)
        );
        
        if (shouldHighlight) {
          const cssClass = normalizedPos === 'noun' ? 'adhd-n' : 
                          normalizedPos === 'verb' ? 'adhd-v' : 'adhd-a';
          html += prefix + `<span class="${cssClass}" data-word="${cleanWord}" data-pos="${pos}">${processedToken}</span>`;
        } else {
          html += token;
        }
      }
