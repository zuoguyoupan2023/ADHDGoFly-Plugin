...
(function (root) {
  ...
  function semanticLayout(context, nodes, edges, width, height) {
    ...
    laneIds.forEach((lane, li) => { const laneNodes = perLane[lane]; laneNodes.sort((a, b) => order.indexOf(a.id) - order.indexOf(b.id)); const step = Math.max(164, (width - 220) / Math.max(1, laneNodes.length)); laneNodes.forEach((node, i) => { positions[node.id] = { ...node, x: 118 + (i + 0.5) * step, y: 112 + li * laneHeight + laneHeight / 2, _lane: lane, _group: li }; }); });
