...
(function (root) {
  ...
  function ensureLaneNodes(context) {
    ...
    Object.entries(laneCounts).forEach(([lane, count], index) => { const rows = Math.max(1, Math.ceil(count / 4)); for (let row = 0; row < rows; row++) { const label = rows > 1 ? `${lane} ${row + 1}` : lane; if (!nodes.some(n => n.isLane && n.laneKey === lane && n.rowIndex === row && !n.hidden)) nodes.push({ id: `lane-${Date.now().toString(36)}-${index}-${row}`, label, laneKey: lane, rowIndex: row, description: '泳道', isLane: true, sourceRefs: [] }); } });
