  ...
  function renderSvg(context) {
    ...
    if (context.intent === 'timeline') {
      ...
      laidNodes.forEach(node => {
        ...
        body += `<g class="agf-chart-node" data-node-id="${esc(node.id)}" style="cursor:grab"><rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${w.toFixed(1)}" height="${h.toFixed(1)}" rx="18" fill="${group.fill}" stroke="${group.line}" stroke-width="2"/><text x="${node.x}" y="${(node.y - 8).toFixed(1)}" text-anchor="middle" font-size="${Math.max(13, 15 * scale).toFixed(1)}" font-family="Arial,sans-serif" font-weight="700" fill="${group.text}" pointer-events="none">${esc(truncate(node.label, 18))}</text><text x="${node.x}" y="${(node.y + 15).toFixed(1)}" text-anchor="middle" font-size="11" font-family="Arial,sans-serif" fill="#687386" pointer-events="none">${esc(truncate(node.description || '', 24))}</text></g>`;
      });
    }
