...
(function (root) {
  ...
  function semanticLayout(context, nodes, edges, width, height) {
    ...
    const grouped = {}; nodes.forEach(n => { const lane = n.groupId || (view === 'data_flow' ? roleOf(n) : '主流程'); (grouped[lane] ||= []).push(n); });
    ...
    rows.forEach(row => row.nodes.forEach(node => { if (Number.isFinite(Number(node.x)) && Number.isFinite(Number(node.y))) positions[node.id].x = Number(node.x), positions[node.id].y = Number(node.y); }));
  ...
  function renderSemanticSvg(context) {
    ...
    layout.rows.forEach((row, i) => { const y = 82 + i * layout.rowHeight; body += `<rect x="24" y="${y.toFixed(1)}" width="952" height="${Math.max(70, layout.rowHeight - 10).toFixed(1)}" rx="12" fill="${theme.lane}" opacity=".72"/><text x="42" y="${(y + 24).toFixed(1)}" font-size="12" font-family="Arial,sans-serif" font-weight="700" fill="${theme.muted}">${esc(row.lane)}${layout.rows.filter(item => item.lane === row.lane).length > 1 ? ` · ${i + 1}` : ''}</text>`; });
