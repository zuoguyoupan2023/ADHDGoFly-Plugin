...
(function (root) {
  ...
  function createFactCheckModule({ context, task, onResult, languageHint } = {}) {
    return { name: 'fact-check', async run() { const ctx = await context.resolve('full_article'); const base = `Identify claims in the article. Return only a strict JSON object, with no explanation and no Markdown code fence. Output at most the 6 most important claims. The required format is {"claims":[{"text":"...","classification":"fact","evidence":"...","reason":"...","confidence":0.8,"locator":"..."}]}. classification must be one of fact/inference/opinion/unverified. fact means explicitly stated by the article, inference means a conclusion derived from the article, opinion means an evaluation or stance, and unverified means the article claims it but does not provide enough evidence or it requires external verification. Do not treat an article opinion as a verified fact. ${languageHint?.() || 'Write all user-facing JSON string values in English.'}`; const material = String(ctx.text || ''); let output; let lastError; for (const attempt of [0, 1]) { try { output = await task.requestJsonText({ prompt: `${base}\n\nArticle:\n${material.slice(0, attempt ? 22000 : 42000)}`, maxTokens: attempt ? 5200 : 3600, temperature: .2 }); if (String(output || '').trim()) break; } catch (error) { lastError = error; if (!/finish_reason:\s*length|AI 返回了空内容/i.test(String(error.message || error)) || attempt) throw error; } } if (!String(output || '').trim()) throw lastError || new Error('事实辨识没有返回结果'); let result = parseObjectPayload(output); if (Array.isArray(result)) result = { claims: result }; if (!result || typeof result !== 'object') throw new Error('事实辨识结果不是对象'); result.claims = Array.isArray(result.claims) ? result.claims : Array.isArray(result.items) ? result.items : []; result.claims = result.claims.filter(item => item && ['fact', 'inference', 'opinion', 'unverified'].includes(item.classification)).map(item => ({ ...item, confidence: Math.max(0, Math.min(1, Number(item.confidence) || 0)) })); onResult?.(result, ctx); return { result, context: ctx }; } };
  }
