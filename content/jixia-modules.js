...
(function (root) {
  ...
  function createFactCheckModule({ context, task, onResult } = {}) {
    return { name: 'fact-check', async run() { const ctx = await context.resolve('full_article'); const output = await task.requestJsonText({ prompt: `请识别文章中的陈述，只返回一个JSON对象，不要解释、不要Markdown代码块。格式必须是 {"claims":[{"text":"...","classification":"fact","evidence":"...","reason":"...","confidence":0.8,"locator":"..."}]}。classification 只能是 fact/inference/opinion/unverified。fact=文章明确陈述的事实，inference=由文章推导出的结论，opinion=评价/立场，unverified=文章声称但缺乏足够依据或需要外部核验的说法。不要把文章观点当成已验证事实。\n\n文章：\n${String(ctx.text || '').slice(0, 60000)}`, maxTokens: 3000, temperature: .2 }); let result = parseObjectPayload(output); if (Array.isArray(result)) result = { claims: result }; if (!result || typeof result !== 'object') throw new Error('事实辨识结果不是对象'); result.claims = Array.isArray(result.claims) ? result.claims : Array.isArray(result.items) ? result.items : []; result.claims = result.claims.filter(item => item && ['fact', 'inference', 'opinion', 'unverified'].includes(item.classification)).map(item => ({ ...item, confidence: Math.max(0, Math.min(1, Number(item.confidence) || 0)) })); onResult?.(result, ctx); return { result, context: ctx }; } };
  }
