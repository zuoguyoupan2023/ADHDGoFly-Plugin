...
class PopupController {
  ...
  showPage(pageId) {
    ...
    if (targetPage) {
      ...
      // 如果是词典编辑页面，初始化编辑器
      if (pageId === 'dict-editor') {
        console.log('Switching to dict editor page...');
        setTimeout(() => {
          this.initDictEditor();
        }, 50);
      }
  ...
  bindUserDictEvents() {
    ...
    // 快速添加词汇确认
    const confirmAddWordBtn = document.getElementById('confirm-add-word-btn');
    if (confirmAddWordBtn) {
    ...
    tabBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        this.switchImportTab(btn.dataset.tab);
      });
    });
    ...
    // 保存词典按钮
    const saveDictEditorBtn = document.getElementById('save-dict-editor-btn');
    if (saveDictEditorBtn) {
      saveDictEditorBtn.addEventListener('click', () => {
        this.saveDictionary();
      });
    }
    ...
    if (jsonFile) {
      jsonFile.addEventListener('change', (e) => {
        this.handleFileUpload(e, 'json');
      });
    }
  }

  // UI方法（暂时只实现界面切换，不实现后台逻辑）
  showDictEditor(dictId = null) {
    // 切换到词典编辑页面
    this.showPage('dict-editor');
    
    // 设置标题
    const title = document.getElementById('editor-title');
    if (title) {
      title.textContent = dictId ? '编辑词典' : '创建词典';
    }

    // 如果是编辑模式，加载词典数据（暂时跳过）
    if (dictId) {
      console.log('编辑词典:', dictId);
      // TODO: 加载词典数据到表单
    } else {
      // 清空表单
      this.clearDictForm();
  ...
  clearDictForm() {
    document.getElementById('dict-name-zh').value = '';
    document.getElementById('dict-name-en').value = '';
    document.getElementById('dict-language').value = '';
    document.getElementById('dict-domain').value = 'general';
    document.getElementById('dict-desc-zh').value = '';
    document.getElementById('dict-desc-en').value = '';
    document.getElementById('dict-author').value = '';
    
    // 清空词汇列表
    const wordsList = document.getElementById('words-list');
    const emptyWords = document.getElementById('empty-words');
    if (wordsList && emptyWords) {
      wordsList.innerHTML = '';
  ...
  showQuickAddForm() {
    ...
    if (quickAddSection) {
      ...
      document.getElementById('quick-word-input').focus();
    }
  }
  ...
  hideQuickAddForm() {
    ...
    if (quickAddSection) {
      ...
      // 清空表单
      document.getElementById('quick-word-input').value = '';
      const checkboxes = document.querySelectorAll('.pos-checkboxes input[type="checkbox"]');
      checkboxes.forEach(cb => cb.checked = false);
    }
  }

  confirmAddWord() {
    const word = document.getElementById('quick-word-input').value.trim();
    const checkboxes = document.querySelectorAll('.pos-checkboxes input[type="checkbox"]:checked');
    const pos = Array.from(checkboxes).map(cb => cb.value);
    ...
    // TODO: 添加词汇到列表（暂时只显示提示）
    console.log('添加词汇:', word, pos);
    alert(`词汇 "${word}" 已添加 (${pos.join(', ')})`);
  ...
  switchImportTab(tabName) {
    ...
    // 切换标签页内容
    const tabContents = document.querySelectorAll('.import-tab');
  ...
  previewImport() {
    ...
    if (preview) {
      ...
      // 模拟统计数据
      document.getElementById('valid-count').textContent = '5';
      document.getElementById('duplicate-count').textContent = '1';
  ...
  saveDictionary() {
    // 获取表单数据
    const nameZh = document.getElementById('dict-name-zh').value.trim();
    const nameEn = document.getElementById('dict-name-en').value.trim();
    const language = document.getElementById('dict-language').value;
    const domain = document.getElementById('dict-domain').value;
    const descZh = document.getElementById('dict-desc-zh').value.trim();
    const descEn = document.getElementById('dict-desc-en').value.trim();
    const author = document.getElementById('dict-author').value.trim();

    // 基本验证
    if (!nameZh) {
      this.showFieldError('dict-name-error', '请输入中文名称');
      return;
    }

    if (!language) {
      this.showFieldError('dict-language-error', '请选择目标语言');
      return;
    }

    // TODO: 保存词典逻辑
    console.log('保存词典:', {
      nameZh, nameEn, language, domain, descZh, descEn, author
    });
  ...
  updateWordCount(count) {
    ...
    if (wordCountElement) {
      wordCountElement.textContent = `(${count}词)`;
    }
  }

  loadUserDictionaries() {
    // TODO: 从存储加载用户词典列表
    console.log('加载用户词典列表');
    
    // 暂时显示空状态
    const container = document.getElementById('user-dicts-container');
    const emptyDiv = document.getElementById('empty-user-dicts');
    const countElement = document.getElementById('user-dict-count');
    
    if (container && emptyDiv && countElement) {
      // 清空容器
      container.innerHTML = '';
      container.appendChild(emptyDiv);
      countElement.textContent = '(0个)';
    }
  }

  initDictEditor() {
    // 初始化词典编辑器
    console.log('初始化词典编辑器');
