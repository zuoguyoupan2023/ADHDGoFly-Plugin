...
class PopupController {
  ...
  async handleDictImport(file) {
    try {
      ...
      // 验证词典格式
      const validation = this.validateDictionary(dictData);
      if (!validation.valid) {
        alert(`词典格式错误: ${validation.error}`);
        return;
      }
  ...
  validateDictionary(dictData) {
    ...
    // 检查必需的meta信息
    if (!dictData.meta) {
      return { valid: false, error: '缺少meta信息' };
    }
    ...
    // 验证语言代码
    const validLanguages = ['zh', 'en', 'es', 'fr', 'ja', 'ru'];
    if (!validLanguages.includes(meta.language.toLowerCase())) {
      return { valid: false, error: `不支持的语言: ${meta.language}` };
    }
  ...
  async updateImportedDictsDisplay() {
    ...
    if (importedDicts.length === 0) {
      importedList.innerHTML = '<div class="imported-dict-empty">暂无导入的词典</div>';
      return;
    }
